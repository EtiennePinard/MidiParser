...
class MidiFile(val headerChunk: HeaderChunk, val midiSequence: MidiSequence) {
    ...
    fun expandedStringRepresentation(): String {
        ...
        for (i in midiSequence.tracks.indices) {
            ...
            midiTrackEvents.events.forEach {
                builder.append("\t\t$it\n")
            }
        }
...
private fun extractTrackChunk(stream: InputStream): TrackChunk {
    if (stream.available() < 8) throw IllegalArgumentException("The stream is less than 0, the minimum length of a track chunk")
    ...
    return if (trackSize == endOfTrackLength) // The midi file is a good file, both numbers match
        TrackChunk(skippedBytes.plus(ByteArrayInputStream(outputStream.toByteArray()).readNBytes(trackSize)), trackSize.toUInt())
    else if (endOfTrackLength != -1) // The midi file is not a good file, since the track size is lying
        TrackChunk(skippedBytes.plus(ByteArrayInputStream(outputStream.toByteArray()).readNBytes(endOfTrackLength)), endOfTrackLength.toUInt())
...
private fun extractMidiEventsFromByteArray(trackData: ByteArray): List<MidiTrackEvent> {
    ...
    while (i < trackData.size) {
        ...
        } else trackData[i].toUByte().toInt()
            ...
            0b1111 -> {
                ...
                when (currentStatus) {
                    ...
                    0xF0, 0xF7 -> { // System exclusive message
                        ...
                        throw NonValidMidiDataException("The byte of this midi, ${currentStatus.toByte().toHexString()}, event which starts with 0b1111, does not match any Sysex or Meta Midi Message!")
                    }
                }
            }
            else -> {
                throw NonValidMidiDataException("The status byte of this midi event, which is ${currentStatus.toByte().toHexString()} is not a valid!")
            }
        }
 ...
 */
private fun ByteArray.subArray(start: Int, end: Int): ByteArray {
    if (end<= start) {
        return ByteArray(0)
    }
 ...
 */
data class VariableLengthQuantity(val quantity: UInt, val length: Int) {
    init {
